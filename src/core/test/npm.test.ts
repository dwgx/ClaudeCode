...
describe("Npm.sanitize", () => {
  test("keeps normal scoped package specs unchanged", () => {
    expect(Npm.sanitize("@claudecode/acme")).toBe("@claudecode/acme")
    expect(Npm.sanitize("@claudecode/acme@1.0.0")).toBe("@claudecode/acme@1.0.0")
    expect(Npm.sanitize("prettier")).toBe("prettier")
  })

  test("handles git https specs", () => {
    const spec = "acme@git+https://github.com/claudecode/acme.git"
    const expected = win ? "acme@git+https_//github.com/claudecode/acme.git" : spec
