...
- Author package entrypoint is `@dwgx/claudecode-plugin/tui`.
...
```json
{
  "$schema": "https://github.com/dwgx/ClaudeCode/tui.json",
  "theme": "smoke-theme",
  "plugin": ["@acme/claudecode-plugin@1.2.3", ["./plugins/demo.tsx", { "label": "demo" }]],
  "plugin_enabled": {
    "acme.demo": false
  }
}
...
- Import types from `@dwgx/claudecode-plugin/tui`.
- `@dwgx/claudecode-plugin` exports `./tui` and declares optional peer deps on `@opentui/core` and `@opentui/solid`.

Minimal module shape:

```tsx
/** @jsxImportSource @opentui/solid */
import type { TuiPlugin, TuiPluginModule } from "@dwgx/claudecode-plugin/tui"
...
- The value is a semver range checked against the running ClaudeCode version.
- If the range is not satisfied, the plugin is skipped with a warning and a session error.
- If `engines.claudecode` is absent, no check is performed (backward compatible).
...
- `claudecode plugin <module>` and TUI install both run install → manifest read → config patch.
- Alias: `claudecode plug <module>`.
- `-g` / `--global` writes into the global config dir.
- Local installs resolve target dir inside `patchPluginConfig`.
- For local scope, path is `<worktree>/.claudecode` only when VCS is git and `worktree !== "/"`; otherwise `<directory>/.claudecode`.
- Root-worktree fallback (`worktree === "/"` uses `<directory>/.claudecode`) is covered by regression tests.
...
When `plugin` entries exist in a writable `.claudecode` dir or `CLAUDECODE_CONFIG_DIR`, ClaudeCode installs `@dwgx/claudecode-plugin` into that dir and writes:
...
That is what makes local config-scoped plugins able to import `@dwgx/claudecode-plugin/tui`.
...
- Local plugins persist installed themes under the local `.claudecode/themes` area near the plugin config source.
...
- `--pure` / `CLAUDECODE_PURE` skips external TUI plugins only.
...
- Local smoke plugin: `.claudecode/plugins/tui-smoke.tsx`
- Local vim plugin: `.claudecode/plugins/tui-vim.tsx`
- Local smoke config: `.claudecode/tui.json`
- Local smoke theme: `.claudecode/plugins/smoke-theme.json`
