...
- `CLAUDECODE_EXPERIMENTAL_HTTPAPI` gates the bridge. Default behavior still uses Hono.
...
- Instance context is provided by `httpapi/server.ts` using `directory`, `workspace`, and `x-claudecode-directory`.
...
3. Mount the new paths in `src/server/routes/instance/index.ts` only inside the `CLAUDECODE_EXPERIMENTAL_HTTPAPI` block.
...
8. Run `bun typecheck` from `packages/claudecode`, relevant `bun run test:ci ...` tests from `packages/claudecode`, and `./packages/sdk/js/script/build.ts` from the repo root.
...
2. The route is mounted by default, not only behind `CLAUDECODE_EXPERIMENTAL_HTTPAPI`.
...
- Cover `directory` and `x-claudecode-directory` instance selection.
...
- [x] Bridge selected `HttpApi` routes into Hono behind `CLAUDECODE_EXPERIMENTAL_HTTPAPI`.
