...
Practical reference for extracting a future `packages/server` from the current `packages/claudecode` monolith while `packages/core` is still being migrated to Effect.
...
- `packages/core` - all claudecode services, Effect-first source of truth
- `packages/server` - claudecode server, with separate contract and implementation, still producing `openapi.json`
...
- import from `server` and embed the full claudecode server into an existing Node app
- spawn the CLI and talk to the server through that boundary

## Current state

Everything still lives in `packages/claudecode`.
...
- those slices are mounted into the Hono server behind `CLAUDECODE_EXPERIMENTAL_HTTPAPI`
...
Use `dwgx/opentunnel` as the structural reference for `packages/server`.
...
The intended direction here is the same, but the current `claudecode` package split is earlier in the migration.
...
- the current server host still lives in `packages/claudecode`
...
- if `packages/server` imports services from `packages/claudecode` while `packages/claudecode` imports `packages/server` to host routes, we create a package cycle immediately
...
4. keep `packages/claudecode` as the temporary Hono host
...
- `packages/server` must not import from `packages/claudecode`

Allowed in phase 1:

- `packages/claudecode` imports `packages/server`
...
- `packages/claudecode` shrinks or disappears as package responsibilities are fully split
...
- `packages/claudecode` keeps generating legacy OpenAPI from Hono routes
...
- expose host adapters needed by `packages/claudecode`
...
Keep in `packages/claudecode` for now:
...
- host mounting from `packages/claudecode`
...
- no imports from `packages/claudecode` inside `packages/server`
...
- `packages/server` must still not import from `packages/claudecode`
...
### PR 4. Mount `packages/server` question from `packages/claudecode`

Scope:

- replace local experimental question route wiring in `packages/claudecode`
...
- mount it in parallel from `packages/claudecode`
...
- move adapter selection and server startup out of `packages/claudecode`

Rules:

- do not start this while `packages/server` still depends on `packages/claudecode`
...
- host logic no longer lives in `packages/claudecode`
...
3. the route is mounted from the current host in `packages/claudecode`
...
- `packages/server` imports services or runtime from `packages/claudecode`
- `packages/claudecode` imports route definitions or handlers from `packages/server`

Avoid by:

- keeping phase-1 `packages/server` free of `packages/claudecode` imports
...
- [ ] mount `question` from `packages/claudecode`
...
2. keep `packages/claudecode` as the temporary host
...
If a proposed PR would make `packages/server` import from `packages/claudecode`, stop and restructure the boundary first.
