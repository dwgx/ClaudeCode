import { Slug } from "@dwgx/claudecode-core/util/slug"
...
export function plan(input: { slug: string; time: { created: number } }) {
  const base = Instance.project.vcs
    ? path.join(Instance.worktree, ".claudecode", "plans")
...
export class Service extends Context.Service<Service, Interface>()("@claudecode/Session") {}
...
export const layer: Layer.Layer<Service, never, Bus.Service | Storage.Service> = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const createNext = Effect.fn("Session.createNext")(function* (input: {
      ...
      if (!Flag.CLAUDECODE_EXPERIMENTAL_WORKSPACES) {
...
export function* list(input?: {
  ...
  } else if (input?.scope !== "project" && !Flag.CLAUDECODE_EXPERIMENTAL_WORKSPACES) {
    if (input?.directory) {
