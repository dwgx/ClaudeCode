...
export type PatchDeps = {
  ...
  files: (dir: string, name: "claudecode" | "tui") => string[]
}
...
function patchDir(input: PatchInput) {
  ...
  return path.join(root, ".claudecode")
}

function patchName(kind: Kind): "claudecode" | "tui" {
  if (kind === "server") return "claudecode"
  return "tui"
}
