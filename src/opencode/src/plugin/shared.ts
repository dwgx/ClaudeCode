...
export const DEPRECATED_PLUGIN_PACKAGES = ["claudecode-openai-codex-auth", "claudecode-copilot-auth"]
...
export async function checkPluginCompatibility(target: string, claudecodeVersion: string, pkg?: PluginPackage) {
  if (!semver.valid(claudecodeVersion) || semver.major(claudecodeVersion) === 0) return
  const hit = pkg ?? (await readPluginPackage(target).catch(() => undefined))
  if (!hit) return
  const engines = hit.json.engines
  if (!isRecord(engines)) return
  const range = engines.claudecode
  if (typeof range !== "string") return
  if (!semver.satisfies(claudecodeVersion, range)) {
    throw new Error(`Plugin requires claudecode ${range} but running ${claudecodeVersion}`)
  }
}
