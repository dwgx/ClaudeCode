...
export const create = fn(CreateInput.zod, async (input) => {
  ...
  const env = {
    CLAUDECODE_AUTH_CONTENT: JSON.stringify(await AppRuntime.runPromise(Auth.Service.use((auth) => auth.all()))),
    CLAUDECODE_WORKSPACE_ID: config.id,
    CLAUDECODE_EXPERIMENTAL_WORKSPACES: "true",
...
async function startSync(space: Info) {
  if (!Flag.CLAUDECODE_EXPERIMENTAL_WORKSPACES) return
