...
function headers(req: Request, extra?: HeadersInit) {
  ...
  out.delete("x-claudecode-directory")
  out.delete("x-claudecode-workspace")
  if (!extra) return out
...
const app = (upgrade: UpgradeWebSocket) =>
  new Hono().get(
    "/__workspace_ws",
    upgrade((c) => {
      const url = c.req.header("x-claudecode-proxy-url")
      const queue: Msg[] = []
...
export function websocket(
  ...
  next.set("x-claudecode-proxy-url", websocketTargetURL(target))
  for (const [key, value] of new Headers(extra).entries()) {
    next.set(key, value)
  }
