...
export const AuthMiddleware: MiddlewareHandler = (c, next) => {
  ...
  const password = Flag.CLAUDECODE_SERVER_PASSWORD
  if (!password) return next()
  const username = Flag.CLAUDECODE_SERVER_USERNAME ?? "claudecode"
...
export function CorsMiddleware(opts?: { cors?: string[] }): MiddlewareHandler {
  return cors({
    ...
    origin(input) {
      ...
      if (/^https:\/\/([a-z0-9-]+\.)*claudecode\.ai$/.test(input)) return input
