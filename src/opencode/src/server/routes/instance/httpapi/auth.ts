...
export class Authorization extends HttpApiMiddleware.Service<Authorization>()(
  "@claudecode/ExperimentalHttpApiAuthorization",
  {
    error: Unauthorized,
    security: {
...
function validateCredential<A, E, R>(
  ...
  return Effect.gen(function* () {
    if (!Flag.CLAUDECODE_SERVER_PASSWORD) return yield* effect

    if (credential.username !== (Flag.CLAUDECODE_SERVER_USERNAME ?? "claudecode")) {
      return yield* new Unauthorized({ message: "Unauthorized" })
    }
    if (Redacted.value(credential.password) !== Flag.CLAUDECODE_SERVER_PASSWORD) {
      return yield* new Unauthorized({ message: "Unauthorized" })
    }
    return yield* effect
  })
