...
>()("@claudecode/ExperimentalHttpApiInstanceContext") {}
...
function provideRequestContext(
  ...
  return Effect.gen(function* () {
    ...
    const envWorkspaceID = Flag.CLAUDECODE_WORKSPACE_ID ? WorkspaceID.make(Flag.CLAUDECODE_WORKSPACE_ID) : undefined
    ...
    const raw = url.searchParams.get("directory") || headers.get("x-claudecode-directory") || currentDirectory()
    const ctx = yield* Effect.promise(() =>
      Instance.provide({
