...
import type { AssistantMessage, Event, OpencodeClient, SessionMessageResponse, ToolPart } from "@dwgx/claudecode-sdk/v2"
...
export class Agent implements ACPAgent {
  ...
  async initialize(params: InitializeRequest): Promise<InitializeResponse> {
    ...
    const authMethod: AuthMethod = {
      description: "Run `claudecode auth login` in the terminal",
      name: "Login with claudecode",
      id: "claudecode-login",
    }
  ...
  private async processMessage(message: SessionMessageResponse) {
    ...
    for (const part of message.parts) {
      if (part.type === "tool") {
        ...
        // ClaudeCode stores files internally as { type: "file", url, filename, mime }.
...
async function defaultModel(config: ACPConfig, cwd?: string): Promise<{ providerID: ProviderID; modelID: ModelID }> {
  ...
  const claudecodeProvider = providers.find((p) => p.id === "claudecode")
  if (claudecodeProvider) {
    if (claudecodeProvider.models["big-pickle"]) {
      return { providerID: ProviderID.claudecode, modelID: ModelID.make("big-pickle") }
    }
    const [best] = Provider.sort(Object.values(claudecodeProvider.models))
    if (best) {
      return {
