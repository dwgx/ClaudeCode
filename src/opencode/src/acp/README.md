...
This directory contains a clean, protocol-compliant implementation of the [Agent Client Protocol](https://agentclientprotocol.com/) for claudecode.
...
- **`session.ts`** - Session state management
  ...
  - Maps ACP sessions to internal claudecode sessions
...
claudecode acp --cwd /path/to/project
```
...
```bash
CLAUDECODE_ENABLE_QUESTION_TOOL=1 claudecode acp
```
...
- **Terminal Integration**: Full terminal support via claudecode's bash tool
...
echo '{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":1}}' | claudecode acp
```
...
### Mapping to ClaudeCode

ACP sessions map cleanly to claudecode's internal session model:
