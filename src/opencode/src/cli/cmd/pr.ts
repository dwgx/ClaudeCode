...
export const PrCommand = cmd({
  command: "pr <number>",
  describe: "fetch and checkout a GitHub PR branch, then run claudecode",
  builder: (yargs) =>
      ...
      async fn() {
        ...
        if (prInfoResult.code === 0) {
          ...
          if (prInfoText.trim()) {
            ...
            // Check for claudecode session link in PR body
            if (prInfo && prInfo.body) {
              const sessionMatch = prInfo.body.match(/https:\/\/opncd\.ai\/s\/([a-zA-Z0-9_-]+)/)
              if (sessionMatch) {
                const sessionUrl = sessionMatch[0]
                UI.println(`Found claudecode session: ${sessionUrl}`)
                UI.println(`Importing session...`)

                const importResult = await Process.text(["claudecode", "import", sessionUrl], {
                  nothrow: true,
                })
        ...
        UI.println()
        UI.println("Starting claudecode...")
        UI.println()

        const claudecodeArgs = sessionId ? ["-s", sessionId] : []
        const claudecodeProcess = Process.spawn(["claudecode", ...claudecodeArgs], {
          stdin: "inherit",
          stdout: "inherit",
          stderr: "inherit",
          cwd: process.cwd(),
        })
        const code = await claudecodeProcess.exited
        if (code !== 0) throw new Error(`claudecode exited with code ${code}`)
      },
    })
  },
})
