...
import claudecode from "./theme/claudecode.json" with { type: "json" }
...
import type { TuiThemeCurrent } from "@dwgx/claudecode-plugin/tui"
  ...
  init: (props: { mode: "dark" | "light" }) => {
    ...
    setStore(
      produce((draft) => {
        ...
        draft.mode = mode
        draft.lock = lock
        const active = config.theme ?? kv.get("theme", "claudecode")
        draft.active = typeof active === "string" ? active : "claudecode"
        draft.ready = false
      }),
    )
    ...
    const values = createMemo(() => {
      ...
      return resolveTheme(store.themes.claudecode, store.mode)
    })
