...
function rendererConfig(_config: TuiConfig.Info): CliRendererConfig {
  const mouseEnabled = !Flag.CLAUDECODE_DISABLE_MOUSE && (_config.mouse ?? true)
...
function App(props: { onSnapshot?: () => Promise<string[]> }) {
  ...
  useKeyboard((evt) => {
    if (!Flag.CLAUDECODE_EXPERIMENTAL_DISABLE_COPY_ON_SELECT) return
  ...
  createEffect(() => {
    if (!terminalTitleEnabled() || Flag.CLAUDECODE_DISABLE_TERMINAL_TITLE) return

    if (route.data.type === "home") {
      renderer.setTerminalTitle("ClaudeCode")
      return
    }
  ...
  command.register(() => [
    ...
    },
    {
      ...
      value: "docs.open",
      onSelect: () => {
        open("https://github.com/dwgx/ClaudeCode/docs").catch(() => {})
        dialog.clear()
      },
      category: "System",
    },
  ...
  event.on("installation.update-available", async (evt) => {
    ...
    await DialogAlert.show(
      dialog,
      "Update Complete",
      `Successfully updated to ClaudeCode v${result.data.version}. Please restart the application.`,
    )
  ...
  return (
    <box
      ...
      onMouseUp={Flag.CLAUDECODE_EXPERIMENTAL_DISABLE_COPY_ON_SELECT ? undefined : () => Selection.copy(renderer, toast)}
    >
      <Show when={Flag.CLAUDECODE_SHOW_TTFD}>
        <TimeToFirstDraw />
      </Show>
      <Show when={ready()}>
        <Switch>
