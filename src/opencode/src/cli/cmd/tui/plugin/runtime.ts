...
import {
  ...
  type TuiSlotPlugin,
  type TuiTheme,
} from "@dwgx/claudecode-plugin/tui"
...
function createThemeInstaller(
  ...
  return async (file) => {
    ...
    const local_dir =
      path.basename(source_dir) === ".claudecode"
        ? path.join(source_dir, "themes")
        : path.join(source_dir, ".claudecode", "themes")
...
function defaultPluginOrigin(state: RuntimeState, spec: string): ConfigPlugin.Origin {
  return {
    spec,
    scope: "local",
    source: state.api.state.path.config || path.join(state.directory, ".claudecode", "tui.json"),
  }
}
...
async function load(input: { api: Api; config: TuiConfig.Info }) {
  ...
  try {
    await Instance.provide({
      directory: cwd,
      fn: async () => {
        const records = Flag.CLAUDECODE_PURE ? [] : (config.plugin_origins ?? [])
        if (Flag.CLAUDECODE_PURE && config.plugin_origins?.length) {
