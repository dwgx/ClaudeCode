...
const TUI_SCHEMA_URL = "https://github.com/dwgx/ClaudeCode/tui.json"
...
/**
 * Migrates tui-specific keys (theme, keybinds, tui) from claudecode.json files
 * into dedicated tui.json files. Migration is performed per-directory and
 * skips only locations where a tui.json already exists.
 */
export async function migrateTuiConfig(input: MigrateInput) {
  const claudecode = await claudecodeFiles(input)
  for (const file of claudecode) {
...
async function claudecodeFiles(input: { directories: string[]; cwd: string }) {
  const files = [
    ...ConfigPaths.fileInDirectory(Global.Path.config, "claudecode"),
    ...(await Filesystem.findUp(["claudecode.json", "claudecode.jsonc"], input.cwd, undefined, { rootFirst: true })),
  ]
  for (const dir of unique(input.directories)) {
    files.push(...ConfigPaths.fileInDirectory(dir, "claudecode"))
  }
  if (Flag.CLAUDECODE_CONFIG) files.push(Flag.CLAUDECODE_CONFIG)
