...
export const UninstallCommand = {
  command: "uninstall",
  describe: "uninstall claudecode and remove all related files",
  builder: (yargs: Argv) =>
    yargs
      .option("keep-config", {
        alias: "c",
  ...
  handler: async (args: UninstallArgs) => {
    ...
    UI.println(UI.logo("  "))
    UI.empty()
    prompts.intro("Uninstall ClaudeCode")
...
async function showRemovalSummary(targets: RemovalTargets, method: Installation.Method) {
  ...
  if (method !== "curl" && method !== "unknown") {
    const cmds: Record<string, string> = {
      npm: "npm uninstall -g @dwgx/claudecode",
      pnpm: "pnpm uninstall -g @dwgx/claudecode",
      bun: "bun remove -g @dwgx/claudecode",
      yarn: "yarn global remove @dwgx/claudecode",
      brew: "brew uninstall claudecode",
      choco: "choco uninstall claudecode",
      scoop: "scoop uninstall claudecode",
    }
...
async function executeUninstall(method: Installation.Method, targets: RemovalTargets) {
  ...
  if (method !== "curl" && method !== "unknown") {
    ...
    if (cmd) {
      ...
      const result = await Process.run(method === "choco" ? ["choco", "uninstall", "claudecode", "-y", "-r"] : cmd, {
        nothrow: true,
      })
      if (result.code !== 0) {
  ...
  UI.empty()
  prompts.log.success("Thank you for using ClaudeCode!")
}
...
async function getShellConfigFile(): Promise<string | null> {
  ...
  for (const file of candidates) {
    ...
    if (content.includes("# claudecode") || content.includes(".claudecode/bin")) {
      return file
    }
  }
...
async function cleanShellConfig(file: string) {
  ...
  for (const line of lines) {
    ...
    if (skip) {
      skip = false
      if (trimmed.includes(".claudecode/bin") || trimmed.includes("fish_add_path")) {
        continue
      }
    }

    if (
      (trimmed.startsWith("export PATH=") && trimmed.includes(".claudecode/bin")) ||
      (trimmed.startsWith("fish_add_path") && trimmed.includes(".claudecode"))
    ) {
      continue
    }
