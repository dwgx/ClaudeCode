...
import { Module } from "@dwgx/claudecode-core/util/module"
...
export const Vue: Info = {
  ...
  async spawn(root) {
    ...
    const args: string[] = []
    if (!binary) {
      if (Flag.CLAUDECODE_DISABLE_LSP_DOWNLOAD) return
...
export const JDTLS: Info = {
  ...
  async spawn(root) {
    ...
    const dataDir = await fs.mkdtemp(path.join(os.tmpdir(), "claudecode-jdtls-data"))
    return {
      process: spawn(
        java,
        [
