...
Replace runtime Bun shell template-tag usage in `packages/claudecode/src` with a unified `Process` API in `util/process.ts`.
...
- Runtime Bun `$` usage in `packages/claudecode/src` is removed except:
