...
const disableDefault = process.env.CLAUDECODE_DISABLE_DEFAULT_PLUGINS
process.env.CLAUDECODE_DISABLE_DEFAULT_PLUGINS = "1"

const { Plugin } = await import("../../src/plugin/index")
const { PluginLoader } = await import("../../src/plugin/loader")
const { readPackageThemes } = await import("../../src/plugin/shared")
const { Instance } = await import("../../src/project/instance")
const { Npm } = await import("@dwgx/claudecode-core/npm")

afterAll(() => {
  if (disableDefault === undefined) {
    delete process.env.CLAUDECODE_DISABLE_DEFAULT_PLUGINS
    return
  }
  process.env.CLAUDECODE_DISABLE_DEFAULT_PLUGINS = disableDefault
})
...
describe("plugin.loader.shared", () => {
  ...
  test("loads npm server plugin from package ./server export", async () => {
    await using tmp = await tmpdir({
      init: async (dir) => {
        ...
        await Bun.write(path.join(dir, "claudecode.json"), JSON.stringify({ plugin: ["acme-plugin@1.0.0"] }, null, 2))
  ...
  test("rejects npm server export that resolves outside plugin directory", async () => {
    await using tmp = await tmpdir({
      init: async (dir) => {
        ...
        await Bun.write(path.join(dir, "claudecode.json"), JSON.stringify({ plugin: ["acme-plugin"] }, null, 2))
  ...
  test("skips legacy codex and copilot auth plugin specs", async () => {
    await using tmp = await tmpdir({
      init: async (dir) => {
        await Bun.write(
          ...
          JSON.stringify(
            {
              plugin: ["claudecode-openai-codex-auth@1.0.0", "claudecode-copilot-auth@1.0.0", "regular-plugin@1.0.0"],
            },
            null,
            2,
          ),
    ...
    try {
      ...
      expect(pkgs).not.toContain("claudecode-openai-codex-auth@1.0.0")
      expect(pkgs).not.toContain("claudecode-copilot-auth@1.0.0")
    } finally {
      install.mockRestore()
    }
  })
  ...
  test("continues loading plugins when plugin init throws", async () => {
    await using tmp = await tmpdir({
      init: async (dir) => {
        ...
        await Bun.write(path.join(dir, "claudecode.json"), JSON.stringify({ plugin: [file, ok] }, null, 2))
  ...
  test("continues loading plugins when plugin import fails", async () => {
    await using tmp = await tmpdir({
      init: async (dir) => {
        ...
        await Bun.write(path.join(dir, "claudecode.json"), JSON.stringify({ plugin: [missing, ok] }, null, 2))
...
`,
        ...
        await Bun.write(path.join(dir, "claudecode.json"), JSON.stringify({ plugin: [aSpec, bSpec] }, null, 2))
  ...
  test("skips external plugins in pure mode", async () => {
    ...
    const pure = process.env.CLAUDECODE_PURE
    process.env.CLAUDECODE_PURE = "1"
    ...
    try {
      ...
      if (pure === undefined) {
        delete process.env.CLAUDECODE_PURE
      } else {
        process.env.CLAUDECODE_PURE = pure
      }
    }
  })
