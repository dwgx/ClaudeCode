...
import { EffectFlock } from "@dwgx/claudecode-core/util/effect-flock"

import { Instance } from "../../src/project/instance"
import { Auth } from "../../src/auth"
import { Account } from "../../src/account/account"
import { AccessToken, AccountID, OrgID } from "../../src/account/schema"
import { AppFileSystem } from "@dwgx/claudecode-core/filesystem"
import { Env } from "../../src/env"
import { provideTmpdirInstance } from "../fixture/fixture"
import { tmpdir } from "../fixture/fixture"
import { CrossSpawnSpawner } from "@dwgx/claudecode-core/cross-spawn-spawner"
...
import { Global } from "@dwgx/claudecode-core/global"
...
const managedConfigDir = process.env.CLAUDECODE_TEST_MANAGED_CONFIG_DIR!
...
async function writeManagedSettings(settings: object, filename = "claudecode.json") {
  await fs.mkdir(managedConfigDir, { recursive: true })
  await Filesystem.write(path.join(managedConfigDir, filename), JSON.stringify(settings))
}

async function writeConfig(dir: string, config: object, name = "claudecode.json") {
...
async function check(map: (dir: string) => string) {
  ...
  try {
    await writeConfig(globalTmp.path, {
      $schema: "https://github.com/dwgx/ClaudeCode/config.json",
      snapshot: false,
    })
...
test("updates global config and omits empty shell key in json", async () => {
  ...
  try {
    ...
    const writtenConfig = await Filesystem.readJson<{ shell?: string }>(path.join(tmp.path, "claudecode.json"))
...
test("updates global config and omits empty shell key in jsonc", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Filesystem.write(
        path.join(dir, "claudecode.jsonc"),
        JSON.stringify({
          $schema: "https://github.com/dwgx/ClaudeCode/config.json",
          shell: "bash",
          model: "test/model",
  ...
  try {
    ...
    const file = path.join(tmp.path, "claudecode.jsonc")
...
test("ignores legacy tui keys in claudecode config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await writeConfig(dir, {
        $schema: "https://github.com/dwgx/ClaudeCode/config.json",
  ...
  try {
    ...
    await Instance.provide({
      ...
      fn: async () => {
        ...
        const content = await Filesystem.readText(path.join(tmp.path, "claudecode.json"))
...
test("resolves env templates in account config with account token", async () => {
  const originalControlToken = process.env["CLAUDECODE_CONSOLE_TOKEN"]
  ...
  const fakeAccount = Layer.mock(Account.Service)({
    ...
    config: () =>
      Effect.succeed(
        Option.some({
          provider: { claudecode: { options: { apiKey: "{env:CLAUDECODE_CONSOLE_TOKEN}" } } },
        }),
      ),
    ...
    await provideTmpdirInstance(() =>
      Config.Service.use((svc) =>
        Effect.gen(function* () {
          ...
          expect(config.provider?.["claudecode"]?.options?.apiKey).toBe("st_test_token")
        }),
      ),
    ).pipe(Effect.scoped, Effect.provide(layer), Effect.provide(Npm.defaultLayer), Effect.runPromise)
  } finally {
    if (originalControlToken !== undefined) {
      process.env["CLAUDECODE_CONSOLE_TOKEN"] = originalControlToken
    } else {
      delete process.env["CLAUDECODE_CONSOLE_TOKEN"]
    }
  }
})
...
test("throws error for invalid JSON", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Filesystem.write(path.join(dir, "claudecode.json"), "{ invalid json }")
    },
  })
  await Instance.provide({
    directory: tmp.path,
...
test("loads config from .claudecode directory", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const claudecodeDir = path.join(dir, ".claudecode")
      await fs.mkdir(claudecodeDir, { recursive: true })
      const agentDir = path.join(claudecodeDir, "agent")
...
test("agent markdown permission config preserves user key order", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const agentDir = path.join(dir, ".claudecode", "agent")
      await fs.mkdir(agentDir, { recursive: true })
...
test("loads agents from .claudecode/agents (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const claudecodeDir = path.join(dir, ".claudecode")
      await fs.mkdir(claudecodeDir, { recursive: true })

      const agentsDir = path.join(claudecodeDir, "agents")
...
test("loads commands from .claudecode/command (singular)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const claudecodeDir = path.join(dir, ".claudecode")
      await fs.mkdir(claudecodeDir, { recursive: true })

      const commandDir = path.join(claudecodeDir, "command")
...
test("loads commands from .claudecode/commands (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const claudecodeDir = path.join(dir, ".claudecode")
      await fs.mkdir(claudecodeDir, { recursive: true })

      const commandsDir = path.join(claudecodeDir, "commands")
...
test("does not try to install dependencies in read-only CLAUDECODE_CONFIG_DIR", async () => {
  if (process.platform === "win32") return
  ...
  const prev = process.env.CLAUDECODE_CONFIG_DIR
  process.env.CLAUDECODE_CONFIG_DIR = tmp.extra
  ...
  try {
    ...
    if (prev === undefined) delete process.env.CLAUDECODE_CONFIG_DIR
    else process.env.CLAUDECODE_CONFIG_DIR = prev
  }
})

test("installs dependencies in writable CLAUDECODE_CONFIG_DIR", async () => {
  await using tmp = await tmpdir<string>({
    init: async (dir) => {
...
test("resolves scoped npm plugins in config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      await Filesystem.write(
        path.join(dir, "claudecode.json"),
        JSON.stringify({ $schema: "https://github.com/dwgx/ClaudeCode/config.json", plugin: ["@scope/plugin"] }, null, 2),
      )
    },
  })
...
test("merges plugin arrays from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      // Create a nested project structure with local .claudecode config
      const projectDir = path.join(dir, "project")
      const claudecodeDir = path.join(projectDir, ".claudecode")
      await fs.mkdir(claudecodeDir, { recursive: true })
      ...
      // Local .claudecode config with different plugins
      await Filesystem.write(
        path.join(claudecodeDir, "claudecode.json"),
        JSON.stringify({
          $schema: "https://github.com/dwgx/ClaudeCode/config.json",
...
test("deduplicates duplicate plugins from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      // Local .claudecode config with some overlapping plugins
      await Filesystem.write(
        path.join(claudecodeDir, "claudecode.json"),
        JSON.stringify({
          $schema: "https://github.com/dwgx/ClaudeCode/config.json",
...
test("keeps plugin origins aligned with merged plugin list", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      const local = path.join(project, ".claudecode")
      ...
      await Filesystem.write(
        path.join(local, "claudecode.json"),
        JSON.stringify({
          $schema: "https://github.com/dwgx/ClaudeCode/config.json",
...
// Note: preload.ts sets CLAUDECODE_TEST_MANAGED_CONFIG which Global.Path.managedConfig uses
...
test("local .claudecode config can override MCP from project config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      // Local .claudecode directory config enables it
      const claudecodeDir = path.join(dir, ".claudecode")
      await fs.mkdir(claudecodeDir, { recursive: true })
      await Filesystem.write(
        path.join(claudecodeDir, "claudecode.json"),
...
test("project config overrides remote well-known config", async () => {
  ...
  globalThis.fetch = mock((url: string | URL | Request) => {
    ...
    if (urlStr.includes(".well-known/claudecode")) {
      fetchedUrl = urlStr
    ...
    await provideTmpdirInstance(
      () =>
        Config.Service.use((svc) =>
          Effect.gen(function* () {
            const config = yield* svc.get()
            expect(fetchedUrl).toBe("https://example.com/.well-known/claudecode")
...
describe("resolvePluginSpec", () => {
  test("keeps package specs unchanged", async () => {
    await using tmp = await tmpdir()
    const file = path.join(tmp.path, "claudecode.json")
    expect(await ConfigPlugin.resolvePluginSpec("oh-my-claudecode@2.4.3", file)).toBe("oh-my-claudecode@2.4.3")
...
describe("deduplicatePluginOrigins", () => {
  ...
  test("keeps path plugins separate from package plugins", () => {
    const plugins = ["oh-my-claudecode@2.4.3", "file:///project/.claudecode/plugin/oh-my-claudecode.js"]
  ...
  test("deduplicates direct path plugins by exact spec", () => {
    const plugins = ["file:///project/.claudecode/plugin/demo.ts", "file:///project/.claudecode/plugin/demo.ts"]

    const result = dedupe(plugins)

    expect(result).toEqual(["file:///project/.claudecode/plugin/demo.ts"])
  })
  ...
  test("loads auto-discovered local plugins as file urls", async () => {
    await using tmp = await tmpdir({
      init: async (dir) => {
        const projectDir = path.join(dir, "project")
        const claudecodeDir = path.join(projectDir, ".claudecode")
        const pluginDir = path.join(claudecodeDir, "plugin")
        await fs.mkdir(pluginDir, { recursive: true })
...
describe("CLAUDECODE_DISABLE_PROJECT_CONFIG", () => {
  test("skips project config files when flag is set", async () => {
    const originalEnv = process.env["CLAUDECODE_DISABLE_PROJECT_CONFIG"]
    process.env["CLAUDECODE_DISABLE_PROJECT_CONFIG"] = "true"
    ...
    try {
      ...
      if (originalEnv === undefined) {
        delete process.env["CLAUDECODE_DISABLE_PROJECT_CONFIG"]
      } else {
        process.env["CLAUDECODE_DISABLE_PROJECT_CONFIG"] = originalEnv
      }
    }
  })

  test("skips project .claudecode/ directories when flag is set", async () => {
    const originalEnv = process.env["CLAUDECODE_DISABLE_PROJECT_CONFIG"]
    process.env["CLAUDECODE_DISABLE_PROJECT_CONFIG"] = "true"
    ...
    try {
      await using tmp = await tmpdir({
        init: async (dir) => {
          // Create a .claudecode directory with a command
          const claudecodeDir = path.join(dir, ".claudecode", "command")
          await fs.mkdir(claudecodeDir, { recursive: true })
          await Filesystem.write(path.join(claudecodeDir, "test-cmd.md"), "# Test Command\nThis is a test command.")
        },
      })
      await Instance.provide({
        directory: tmp.path,
        fn: async () => {
          ...
          // Project .claudecode should NOT be in directories list
  ...
  test("skips relative instructions with warning when flag is set but no config dir", async () => {
    const originalDisable = process.env["CLAUDECODE_DISABLE_PROJECT_CONFIG"]
    const originalConfigDir = process.env["CLAUDECODE_CONFIG_DIR"]

    try {
      // Ensure no config dir is set
      delete process.env["CLAUDECODE_CONFIG_DIR"]
      process.env["CLAUDECODE_DISABLE_PROJECT_CONFIG"] = "true"
      ...
      if (originalDisable === undefined) {
        ...
        process.env["CLAUDECODE_DISABLE_PROJECT_CONFIG"] = originalDisable
      }
      if (originalConfigDir === undefined) {
        delete process.env["CLAUDECODE_CONFIG_DIR"]
      } else {
        process.env["CLAUDECODE_CONFIG_DIR"] = originalConfigDir
      }
    }
  })

  test("CLAUDECODE_CONFIG_DIR still works when flag is set", async () => {
    const originalDisable = process.env["CLAUDECODE_DISABLE_PROJECT_CONFIG"]
    const originalConfigDir = process.env["CLAUDECODE_CONFIG_DIR"]
    ...
    try {
      ...
      process.env["CLAUDECODE_DISABLE_PROJECT_CONFIG"] = "true"
      process.env["CLAUDECODE_CONFIG_DIR"] = configDirTmp.path

      await Instance.provide({
        directory: projectTmp.path,
        fn: async () => {
          const config = await load()
          // Should load from CLAUDECODE_CONFIG_DIR, not project
...
describe("CLAUDECODE_CONFIG_CONTENT token substitution", () => {
  test("substitutes {env:} tokens in CLAUDECODE_CONFIG_CONTENT", async () => {
    const originalEnv = process.env["CLAUDECODE_CONFIG_CONTENT"]
    const originalTestVar = process.env["TEST_CONFIG_VAR"]
    process.env["TEST_CONFIG_VAR"] = "test_api_key_12345"
    process.env["CLAUDECODE_CONFIG_CONTENT"] = JSON.stringify({
      $schema: "https://github.com/dwgx/ClaudeCode/config.json",
    ...
    try {
      ...
      if (originalEnv !== undefined) {
        process.env["CLAUDECODE_CONFIG_CONTENT"] = originalEnv
      } else {
        delete process.env["CLAUDECODE_CONFIG_CONTENT"]
      }
  ...
  test("substitutes {file:} tokens in CLAUDECODE_CONFIG_CONTENT", async () => {
    const originalEnv = process.env["CLAUDECODE_CONFIG_CONTENT"]
  ...
  const config = ConfigParse.effectSchema(
    ...
    ConfigParse.jsonc(
      await ConfigManaged.parseManagedPlist(
        JSON.stringify({
          PayloadDisplayName: "ClaudeCode Managed",
          PayloadIdentifier: "ai.claudecode.managed.test",
          PayloadType: "ai.claudecode.managed",
...
test("parseManagedPlist handles empty config", async () => {
  const config = ConfigParse.effectSchema(
    Config.Info,
    ConfigParse.jsonc(
      await ConfigManaged.parseManagedPlist(JSON.stringify({ $schema: "https://github.com/dwgx/ClaudeCode/config.json" })),
      "test:mobileconfig",
    ),
    "test:mobileconfig",
  )
  expect(config.$schema).toBe("https://github.com/dwgx/ClaudeCode/config.json")
})
