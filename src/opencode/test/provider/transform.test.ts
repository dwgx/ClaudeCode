...
describe("ProviderTransform.message - strip openai metadata when store=false", () => {
  ...
  test("preserves metadata using providerID key when store is false", () => {
    const claudecodeModel = {
      ...
      api: {
        id: "claudecode-test",
        url: "https://api.github.com/dwgx/ClaudeCode",
        npm: "@ai-sdk/openai-compatible",
      },
    }
    const msgs = [
    ...
    const result = ProviderTransform.message(msgs, claudecodeModel, { store: false }) as any[]

    expect(result[0].content[0].providerOptions?.claudecode?.itemId).toBe("msg_123")
    expect(result[0].content[0].providerOptions?.claudecode?.otherOption).toBe("value")
  })
  ...
  test("preserves itemId across all providerOptions keys", () => {
    ...
    const msgs = [
      {
        ...
        providerOptions: {
          ...
          claudecode: { itemId: "msg_claudecode" },
        ...
        content: [
          {
            ...
            providerOptions: {
              ...
              claudecode: { itemId: "msg_claudecode_part" },
    ...
    expect(result[0].providerOptions?.claudecode?.itemId).toBe("msg_claudecode")
    expect(result[0].providerOptions?.extra?.itemId).toBe("msg_extra")
    expect(result[0].content[0].providerOptions?.openai?.itemId).toBe("msg_openai_part")
    expect(result[0].content[0].providerOptions?.claudecode?.itemId).toBe("msg_claudecode_part")
