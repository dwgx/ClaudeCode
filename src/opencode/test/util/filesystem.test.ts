...
describe("filesystem", () => {
  ...
  describe("findUp()", () => {
    ...
    test("rootFirst preserves json then jsonc order per directory", async () => {
      ...
      await fs.writeFile(path.join(tmp.path, "claudecode.json"), "{}", "utf-8")
      await fs.writeFile(path.join(tmp.path, "claudecode.jsonc"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "claudecode.json"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "claudecode.jsonc"), "{}", "utf-8")

      const result = await Filesystem.findUp(["claudecode.json", "claudecode.jsonc"], nested, tmp.path, {
        rootFirst: true,
      })

      expect(result).toEqual([
        path.join(tmp.path, "claudecode.json"),
        path.join(tmp.path, "claudecode.jsonc"),
        path.join(project, "claudecode.json"),
        path.join(project, "claudecode.jsonc"),
      ])
    })
  })
