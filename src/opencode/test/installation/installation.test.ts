...
describe("installation", () => {
  describe("latest", () => {
    ...
    test("reads npm versions via registry", async () => {
      ...
      expect(calls).toContain(`https://registry.npmjs.org/@dwgx/claudecode/${InstallationChannel}`)
    })
    ...
    test("reads brew formulae API versions", async () => {
      const layer = testLayer(
        ...
        (cmd, args) => {
          ...
          if (cmd === "brew" && args.includes("--formula") && args.includes("dwgx/tap/claudecode")) return ""
          if (cmd === "brew" && args.includes("--formula") && args.includes("claudecode")) return "claudecode"
          return ""
        },
      )
    ...
    test("reads brew tap info JSON via CLI", async () => {
      ...
      const layer = testLayer(
        ...
        (cmd, args) => {
          if (cmd === "brew" && args.includes("dwgx/tap/claudecode") && args.includes("--formula")) return "claudecode"
