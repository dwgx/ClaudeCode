...
const originalWorkspaces = Flag.CLAUDECODE_EXPERIMENTAL_WORKSPACES
...
afterEach(async () => {
  Flag.CLAUDECODE_EXPERIMENTAL_WORKSPACES = originalWorkspaces
  await Instance.disposeAll()
})

describe("session.list", () => {
  test("does not filter by directory when directory is omitted", async () => {
    Flag.CLAUDECODE_EXPERIMENTAL_WORKSPACES = false
    await using tmp = await tmpdir({ git: true })
    await mkdir(path.join(tmp.path, "packages", "claudecode"), { recursive: true })
    ...
    await Instance.provide({
      ...
      fn: async () => {
        ...
        const current = await Instance.provide({
          directory: path.join(tmp.path, "packages", "claudecode"),
  ...
  test("filters by directory when directory is provided", async () => {
    ...
    await Instance.provide({
      ...
      fn: async () => {
        ...
        const ids = [...svc.list({ directory: path.join(tmp.path, "packages", "claudecode") })].map((s) => s.id)
  ...
  test("filters by path and ignores directory when path is provided", async () => {
    Flag.CLAUDECODE_EXPERIMENTAL_WORKSPACES = false
    await using tmp = await tmpdir({ git: true })
    await mkdir(path.join(tmp.path, "packages", "claudecode", "src", "deep"), { recursive: true })
    await mkdir(path.join(tmp.path, "packages", "app"), { recursive: true })
    ...
    await Instance.provide({
      ...
      fn: async () => {
        ...
        const current = await Instance.provide({
          directory: path.join(tmp.path, "packages", "claudecode", "src"),
          fn: async () => svc.create({ title: "current" }),
        })
        const deeper = await Instance.provide({
          directory: path.join(tmp.path, "packages", "claudecode", "src", "deep"),
        ...
        const pathIDs = [
          ...svc.list({ directory: path.join(tmp.path, "packages", "app"), path: "packages/claudecode/src" }),
        ].map((s) => s.id)
  ...
  test("falls back to directory when filtering legacy sessions without path", async () => {
    Flag.CLAUDECODE_EXPERIMENTAL_WORKSPACES = false
    await using tmp = await tmpdir({ git: true })
    await mkdir(path.join(tmp.path, "packages", "claudecode", "src"), { recursive: true })
    await mkdir(path.join(tmp.path, "packages", "app"), { recursive: true })
    ...
    await Instance.provide({
      ...
      fn: async () => {
        ...
        const pathIDs = [
          ...svc.list({ directory: path.join(tmp.path, "packages", "claudecode", "src"), path: "packages/claudecode/src" }),
        ].map((s) => s.id)
        expect(pathIDs).toContain(current.id)
        expect(pathIDs).not.toContain(sibling.id)
      },
