...
import { createOpencodeClient } from "@dwgx/claudecode-sdk/v2"
...
function app(backend: Backend, input?: { password?: string; username?: string }) {
  Flag.CLAUDECODE_EXPERIMENTAL_HTTPAPI = backend === "httpapi"
      ...
      Effect.gen(function* () {
        ...
        const bad = yield* capture(() =>
          client(backend, directory, {
            password: "secret",
            headers: { authorization: authorization("claudecode", "wrong") },
          }).file.read({ path: "hello.txt" }),
        )
        const good = yield* capture(() =>
          client(backend, directory, {
            password: "secret",
            headers: { authorization: authorization("claudecode", "secret") },
          }).file.read({ path: "hello.txt" }),
        )
