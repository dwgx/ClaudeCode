...
const original = Flag.CLAUDECODE_EXPERIMENTAL_HTTPAPI

function app() {
  Flag.CLAUDECODE_EXPERIMENTAL_HTTPAPI = true
...
afterEach(async () => {
  Flag.CLAUDECODE_EXPERIMENTAL_HTTPAPI = original
...
describe("instance HttpApi", () => {
  test("serves path and VCS read endpoints through Hono bridge", async () => {
    ...
    const [paths, vcs, diff] = await Promise.all([
      app().request(InstancePaths.path, { headers: { "x-claudecode-directory": tmp.path } }),
      app().request(InstancePaths.vcs, { headers: { "x-claudecode-directory": tmp.path } }),
      app().request(vcsDiff, { headers: { "x-claudecode-directory": tmp.path } }),
    ])
  ...
  test("serves catalog read endpoints through Hono bridge", async () => {
    ...
    const [commands, agents, skills, lsp, formatter] = await Promise.all([
      app().request(InstancePaths.command, { headers: { "x-claudecode-directory": tmp.path } }),
      app().request(InstancePaths.agent, { headers: { "x-claudecode-directory": tmp.path } }),
      app().request(InstancePaths.skill, { headers: { "x-claudecode-directory": tmp.path } }),
      app().request(InstancePaths.lsp, { headers: { "x-claudecode-directory": tmp.path } }),
      app().request(InstancePaths.formatter, { headers: { "x-claudecode-directory": tmp.path } }),
    ])
  ...
  test("serves project git init through Hono bridge", async () => {
    ...
    const response = await app().request("/project/git/init", {
      method: "POST",
      headers: { "x-claudecode-directory": tmp.path },
    })
    ...
    const current = await app().request("/project/current", { headers: { "x-claudecode-directory": tmp.path } })
  ...
  test("serves project update through Hono bridge", async () => {
    ...
    const response = await app().request(`/project/${project.id}`, {
      method: "PATCH",
      headers: { "x-claudecode-directory": tmp.path, "content-type": "application/json" },
    ...
    const list = await app().request("/project", { headers: { "x-claudecode-directory": tmp.path } })
