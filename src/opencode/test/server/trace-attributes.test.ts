...
describe("paramToAttributeKey", () => {
  ...
  test("namespaces non-ID params under claudecode.", () => {
    expect(paramToAttributeKey("name")).toBe("claudecode.name")
    expect(paramToAttributeKey("slug")).toBe("claudecode.slug")
  })
})
...
describe("requestAttributes", () => {
  ...
  test("emits OTel-style <domain>.id for ID-shaped route params", () => {
    ...
    expect(attrs["claudecode.sessionID"]).toBeUndefined()
    expect(attrs["claudecode.messageID"]).toBeUndefined()
    expect(attrs["claudecode.partID"]).toBeUndefined()
  })
  ...
  test("namespaces non-ID params under claudecode. (e.g. mcp :name)", () => {
    const attrs = requestAttributes(
      fakeContext("POST", "http://localhost/mcp/exa/connect", {
        name: "exa",
      }),
    )
    expect(attrs["claudecode.name"]).toBe("exa")
