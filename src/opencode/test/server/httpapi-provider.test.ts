...
function app(experimental: boolean) {
  Flag.CLAUDECODE_EXPERIMENTAL_HTTPAPI = experimental
...
function writeProviderAuthPlugin(dir: string) {
  return Effect.gen(function* () {
    ...
    yield* fs.makeDirectory(path.join(dir, ".claudecode", "plugin"), { recursive: true })
    yield* fs.writeFileString(
      path.join(dir, ".claudecode", "plugin", "provider-oauth-parity.ts"),
      [
        "export default {",
...
function withProviderProject<A, E, R>(self: (dir: string) => Effect.Effect<A, E, R>) {
  return Effect.gen(function* () {
    const fs = yield* FileSystem.FileSystem
    const path = yield* Path.Path
    const dir = yield* fs.makeTempDirectoryScoped({ prefix: "claudecode-test-" })

    yield* fs.writeFileString(
      path.join(dir, "claudecode.json"),
      JSON.stringify({ $schema: "https://github.com/dwgx/ClaudeCode/config.json", formatter: false, lsp: false }),
    )
...
describe("provider HttpApi", () => {
  it.live(
    ...
    withProviderProject((dir) =>
      Effect.gen(function* () {
        const headers = { "x-claudecode-directory": dir, "content-type": "application/json" }
