...
const original = {
  CLAUDECODE_EXPERIMENTAL_HTTPAPI: Flag.CLAUDECODE_EXPERIMENTAL_HTTPAPI,
  CLAUDECODE_SERVER_PASSWORD: Flag.CLAUDECODE_SERVER_PASSWORD,
  CLAUDECODE_SERVER_USERNAME: Flag.CLAUDECODE_SERVER_USERNAME,
}
...
function app(input?: { password?: string; username?: string }) {
  ...
  Flag.CLAUDECODE_SERVER_PASSWORD = input?.password
  Flag.CLAUDECODE_SERVER_USERNAME = input?.username
...
afterEach(async () => {
  Flag.CLAUDECODE_EXPERIMENTAL_HTTPAPI = original.CLAUDECODE_EXPERIMENTAL_HTTPAPI
  Flag.CLAUDECODE_SERVER_PASSWORD = original.CLAUDECODE_SERVER_PASSWORD
  Flag.CLAUDECODE_SERVER_USERNAME = original.CLAUDECODE_SERVER_USERNAME
...
describe("HttpApi server", () => {
  test("keeps Effect HttpApi behind the feature flag", () => {
    Flag.CLAUDECODE_EXPERIMENTAL_HTTPAPI = false
  ...
  test("allows requests when auth is disabled", async () => {
    ...
    const response = await app().request(fileUrl(), {
      headers: {
        "x-claudecode-directory": tmp.path,
      },
    })
  ...
  test("requires credentials when auth is enabled", async () => {
    ...
    const [missing, bad, good] = await Promise.all([
      ...
      app({ password: "secret" }).request(fileUrl(), {
        headers: {
          authorization: authorization("claudecode", "wrong"),
          "x-claudecode-directory": tmp.path,
        },
      }),
      app({ password: "secret" }).request(fileUrl(), {
        headers: {
          authorization: authorization("claudecode", "secret"),
          "x-claudecode-directory": tmp.path,
        },
      }),
    ])
  ...
  test("accepts auth_token query credentials", async () => {
    ...
    const response = await app({ password: "secret" }).request(
      fileUrl({ token: Buffer.from("claudecode:secret").toString("base64") }),
      {
        headers: {
          "x-claudecode-directory": tmp.path,
        },
  ...
  test("selects instance from query before directory header", async () => {
    ...
    const response = await app().request(fileUrl({ directory: query.path }), {
      headers: {
        "x-claudecode-directory": header.path,
      },
    })
  ...
  test("serves global health from Effect HttpApi", async () => {
    const response = await app().request(`${GlobalPaths.health}?directory=/does/not/exist/claudecode-test`)
